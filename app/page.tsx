...
async function getProducts(): Promise<{
  ...
  try {
    ...
    const products: ProductCardData[] = (data ?? []).map((p) => {
      const photos = (
        p as { product_photos?: { photo_url: string; is_cover: boolean }[] }
      ).product_photos;
      const cover =
        photos?.find((ph) => ph.is_cover)?.photo_url ??
        photos?.[0]?.photo_url ??
        null;
      const sellerRaw = (
        p as {
          sellers?: { business_name: string; verification_status: string }[];
        }
      ).sellers;
    ...
    <>
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 py-6 w-full">
        {error === "not_configured" && (
          ...
          >
            ...
            <p>
              ...
              add your Project URL and anon key from Supabase → Settings → API,
              then restart the dev server. This page will populate automatically
              once it&apos;s connected.
            </p>
          </div>
        )}
        ...
        {error && error !== "not_configured" && (
          <div
            className="rounded-lg border p-6 mb-8 text-sm"
            style={{ borderColor: "#e0a0a0", background: "#fdf0f0" }}
          >
            <p className="font-medium mb-1">Couldn&apos;t load listings</p>
            <p>
              {error}. Try refreshing. If this keeps happening, check your
              Supabase connection.
            </p>
          </div>
        )}
        ...
        {products.length === 0 && error !== "not_configured" && !error && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">
              Be the first seller in your area. Verification takes just a few
              minutes.
            </p>
            <Link
              href="/signup"
      ...
      <footer
        className="border-t py-6 text-center text-xs text-gray-500"
        style={{ borderColor: "var(--sand)" }}
      >
        Teraa · Wave payments and cash on delivery available ·{" "}
